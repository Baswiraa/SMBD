  ...
  <form action="produk-save.php" method="post" class="space-y-4">
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <div>
        ...
        <input type="number" name="quantity" required class="w-full border p-2 rounded" value="<?= $edit?($conn->query("SELECT quantity FROM stock WHERE product_id={$data['product_id']}")->fetch_row()[0]??0):0 ?>">
      </div>
    </div>
    <div>
